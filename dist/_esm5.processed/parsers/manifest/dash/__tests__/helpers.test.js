...
import { parseByteRange, parseDuration, replaceRepresentationDASHTokens, replaceSegmentDASHTokens, } from "../helpers";
describe("dash parser helpers", function () {
    describe("parseDuration", function () {
        ...
        it("should throw if duration not in ISO8061 format", function () {
            expect(function () { return parseDuration("1000"); }).to.throw();
        });
    });
    describe("parseByteRange", function () {
        it("should correctly parse byte range", function () {
            var parsedByteRange = parseByteRange("1-1000");
            expect(parsedByteRange).not.to.equal(null);
            expect(parsedByteRange.length).to.equal(2);
            expect(parsedByteRange[0]).to.equal(1);
            expect(parsedByteRange[1]).to.equal(1000);
        });
        it("should return null if can't parse given byte range", function () {
            expect(parseByteRange("main")).to.equal(null);
        });
    });
    describe("replaceSegmentDASHTokens", function () {
    ...
    describe("replaceRepresentationDASHTokens", function () {
