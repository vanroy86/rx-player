...
import { catchError, filter, ignoreElements, map, mergeMap, shareReplay, tap, } from "rxjs/operators";
...
export default function EMEManager(mediaElement, keySystemsConfigs) {
    ...
    var mediaKeysInfos$ = initMediaKeys(mediaElement, keySystemsConfigs, attachedMediaKeysInfos)
    ...
    var encryptedEvents$ = onEncrypted$(mediaElement).pipe(tap(function (encryptedEvent) {
        log.debug("EME: encrypted event received", encryptedEvent);
    }));
    var bindSession$ = observableCombineLatest([encryptedEvents$,
        attachedMediaKeys$]).pipe(
    ...
    }), filter(function (evt) { return evt.type === "warning"; }));
