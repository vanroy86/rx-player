...
function licenseErrorSelector(error, fatal) {
    ...
    return new EncryptedMediaError("KEY_LOAD_ERROR", error.message || error.toString(), fatal);
}
/**
...
export default function handleSessionEvents(session, keySystem) {
    ...
    var getLicenseRetryOptions = { totalRetry: 2,
        retryDelay: 200,
    ...
    var keyStatusesChanges = onKeyStatusesChange$(session)
        ...
        session.keyStatuses.forEach(function (keyStatus, keyId) {
            ...
            if (KEY_STATUS_ERRORS[keyId]) {
                throw new EncryptedMediaError("KEY_STATUS_CHANGE_ERROR", "An invalid key status has been " +
                    "encountered: " + keyId, true);
            }
            else if (KEY_STATUS_ERRORS[keyStatus]) {
                throw new EncryptedMediaError("KEY_STATUS_CHANGE_ERROR", "An invalid key status has been " +
                    "encountered: " + keyStatus, true);
            }
        });
        var warnings$ = warnings.length ? observableOf.apply(void 0, warnings) :
            EMPTY;
        ...
        }), map(function (licenseObject) { return ({ type: "key-status-change",
        ...
        var getLicense$ = observableDefer(function () {
            ...
            return castToObservable(getLicense)
                ...
                if (error instanceof TimeoutError) {
                    throw new EncryptedMediaError("KEY_LOAD_TIMEOUT", "The license server took more " +
                        "than 10 seconds to respond.", false);
                }
