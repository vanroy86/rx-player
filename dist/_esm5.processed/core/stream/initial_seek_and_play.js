...
import { catchError, mapTo, mergeMap, multicast, refCount, tap, } from "rxjs/operators";
import { canPlay, hasLoadedMetadata, play$, } from "../../compat";
import log from "../../log";
/**
 ...
 * @returns {object}
 */
export default function seekAndLoadOnMediaEvents(mediaElement, startTime, mustAutoPlay) {
    var seek$ = doInitialSeek(mediaElement, startTime);
    var load$ = canPlay(mediaElement).pipe(tap(function () { return log.info("canplay event"); }), mergeMap(function () {
        if (mustAutoPlay) {
