 ...
 */
import { concat as observableConcat, defer as observableDefer, merge as observableMerge, of as observableOf, race as observableRace, } from "rxjs";
import { concatMapTo, mergeMap, take, takeLast, } from "rxjs/operators";
import { onRemoveSourceBuffers$, onSourceOpen$, onUpdate$, } from "../../compat/events";
import log from "../../log";
/**
...
export default function triggerEndOfStream(mediaSource) {
    return observableDefer(function () {
        ...
        var updatedSourceBuffers$ = updatingSourceBuffers
            .map(onUpdate$);
        return observableRace(observableMerge.apply(void 0, updatedSourceBuffers$).pipe(takeLast(1)), onRemoveSourceBuffers$(sourceBuffers)
            .pipe(take(1))).pipe(mergeMap(function () {
...
export function maintainEndOfStream(mediaSource) {
    return observableConcat(triggerEndOfStream(mediaSource), onSourceOpen$(mediaSource)
