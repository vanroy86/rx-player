...
export default function createEMEManager(mediaElement, keySystems) {
    if (features.emeManager == null) {
        return observableMerge(onEncrypted$(mediaElement).pipe(map(function () {
            ...
            throw new EncryptedMediaError("MEDIA_IS_ENCRYPTED_ERROR", "EME feature not activated.", true);
        })), observableOf({ type: "eme-disabled" }));
    }
    if (!keySystems || !keySystems.length) {
        return observableMerge(onEncrypted$(mediaElement).pipe(map(function () {
            log.error("Init: Ciphered media and no keySystem passed");
            throw new EncryptedMediaError("MEDIA_IS_ENCRYPTED_ERROR", "Media is encrypted and no `keySystems` given", true);
        })), observableOf({ type: "eme-disabled" }));
    }
    if (!hasEMEAPIs()) {
        return observableMerge(onEncrypted$(mediaElement).pipe(map(function () {
            log.error("Init: Encrypted event but no EME API available");
            throw new EncryptedMediaError("MEDIA_IS_ENCRYPTED_ERROR", "Encryption APIs not found.", true);
        })), observableOf({ type: "eme-disabled" }));
    }
