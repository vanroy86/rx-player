...
export default function RepresentationBuffer(_a) {
    ...
    var initSegmentObject = initSegment == null ? { segmentData: null, segmentInfos: null, segmentOffset: 0 } :
        null;
    ...
    var status$ = observableCombineLatest([
    ...
    ]).pipe(map(function getCurrentStatus(_a) {
        ...
        if (initSegment != null && initSegmentObject == null) {
            ...
            neededSegments = [{ segment: initSegment,
                    priority: initSegmentPriority }].concat(neededSegments);
        }
        var isFull = !neededSegments.length &&
            period.end != null &&
        ...
        if (status.discontinuity > 1) {
            // TODO Refacto discontinuity logic
            var seekTo = status.discontinuity + 1;
    ...
    function appendSegment(loadedSegment) {
        return observableDefer(function () {
            ...
            return append$.pipe(mapTo(EVENTS.addedSegment(bufferType, segment, segmentData)), tap(function () {
                ...
                var _a = segmentInfos != null ? segmentInfos :
                    segment, time = _a.time, duration = _a.duration, timescale = _a.timescale;
