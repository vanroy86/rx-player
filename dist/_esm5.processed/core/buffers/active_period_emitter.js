 ...
 * @returns {Observable}
 */
export default function ActivePeriodEmitter(bufferTypes, addPeriodBuffer$, removePeriodBuffer$) {
    var periodsList = new SortedList(function (a, b) { return a.period.start - b.period.start; });
    var onItemAdd$ = addPeriodBuffer$
        .pipe(tap(function (_a) {
        var period = _a.period, type = _a.type;
        // add or update the periodItem
        var periodItem = periodsList.findFirst(function (p) { return p.period === period; });
        if (!periodItem) {
            periodItem = {
                period: period,
                buffers: new Set(),
    ...
    return observableMerge(onItemAdd$, onItemRemove$).pipe(map(function () {
