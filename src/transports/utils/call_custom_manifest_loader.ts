...
export default function callCustomManifestLoader<T>(
  customManifestLoader : CustomManifestLoader<any>, // TODO?
  fallbackManifestLoader : (url? : string) => IManifestLoaderObservable<T>,
  url? : string
) : IManifestLoaderObservable<T> {

  const timeAPIsDelta = Date.now() - performance.now();

  return new Observable((obs: IManifestLoaderObserver<T>) => {
    ...
    const fallback = () => {
      hasFallbacked = true;
      fallbackManifestLoader(url).subscribe(obs);
    };
