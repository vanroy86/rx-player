                 ...
                 IBufferManifestMightBeOutOfSync>
  {
    ...
    return observableConcat(
      ...
      observableDefer(() => { // better if done after warning is emitted
        ...
        } else if (evt.value instanceof NetworkError &&
                   evt.value.isHttpError(404) &&
                   index.canBeOutOfSync())
        {
          return observableOf(EVENTS.manifestMightBeOufOfSync(adaptation.type));
        }
        return EMPTY;
      }));
  }
