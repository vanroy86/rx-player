...
import { IAuthorization } from "../stream/stream_authorization_manager";
...
export default class RepresentationChooser {
  ...
  private readonly _authorizations$ : Observable<IAuthorization[]>;
  ...
  constructor(options : IRepresentationChooserOptions) {
    ...
    this._authorizations$ = options.authorizations$;
  ...
  public get$(
    ...
    const authorizedRepresentations$: Observable<Representation[]> =
      this._authorizations$.pipe(
        map((authorizations) => {
          return authorizations
            .filter((authorization) => {
              const authId = authorization.IDSet.representationId;
              const rep = representations.find((_rep) => _rep.id === authId);
              if (rep) {
                return authorization.isPlayable;
              }
              return false;
            })
            .map((auth) => {
              return representations.find((rep) =>
                rep.id === auth.IDSet.representationId);
            });
        }),
        startWith(representations)
      );

    return authorizedRepresentations$.pipe(
      switchMap((authorizedRepresentations) => {

        if (!authorizedRepresentations.length) {
          throw new Error("ABRManager: no representation choice given");
        }
        if (authorizedRepresentations.length === 1) {
          return observableOf({
            bitrate: undefined, // Bitrate estimation is deactivated here
            representation: authorizedRepresentations[0],
            manual: false,
            urgent: true,
          });
        }
        ...
        return manualBitrate$.pipe(switchMap(manualBitrate => {
          if (manualBitrate >= 0) {
            ...
            return observableOf({
              bitrate: undefined, // Bitrate estimation is deactivated here
              representation: fromBitrateCeil(authorizedRepresentations, manualBitrate) ||
                authorizedRepresentations[0],
              manual: true,
          ...
          return observableCombineLatest(
            ...
            map(([ clock, maxAutoBitrate, deviceEvents ]) => {
              ...
              const _representations =
                getFilteredRepresentations(authorizedRepresentations, deviceEvents);

              const chosenRepresentation = fromBitrateCeil(
                _representations, newBitrateCeil) || authorizedRepresentations[0];
